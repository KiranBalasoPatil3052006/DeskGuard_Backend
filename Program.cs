...
try
{
    using (var scope = app.Services.CreateScope())
    {
        ...
        else
        {
            ...
            existingUser.LockoutEndAt = null;
            existingUser.FailedLoginAttempts = 0;
            existingUser.UpdatedAt = DateTime.UtcNow;
            await dbContext.SaveChangesAsync();
            Log.Information("Updated password, unlocked account, and status for Super Admin user: {Email}", email);
        }

        try
        {
            await dbContext.Database.ExecuteSqlRawAsync($"UPDATE users SET password = '{passwordHash}', lockout_end_at = NULL, failed_login_attempts = 0, is_active = true WHERE LOWER(email) = 'kiranbalasopatil33@gmail.com';");
            await dbContext.Database.ExecuteSqlRawAsync("UPDATE users SET lockout_end_at = NULL, failed_login_attempts = 0;");
            await dbContext.Database.ExecuteSqlRawAsync("UPDATE security_settings SET max_failed_login_attempts = 0, account_lockout_duration_minutes = 0;");
        }
        catch
        {
